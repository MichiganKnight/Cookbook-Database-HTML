...
<body>
    <?php
        ...
        $recipe = $_FILES['saladInput']['name'];
        $splitRecipe = trim($recipe, ".jpg");
        $recipeInput = $db->prepare("INSERT INTO SaladTable (id, Salad) VALUES (?, ?)");
        $recipeInput->execute(array(null, $splitRecipe));
        echo '<h1>The Recipe: ' . $splitRecipe . ' Has Successfully Been Imported!</h1>';
        if (isset($_FILES['saladInput']) && $_FILES['saladInput']['error'] === UPLOAD_ERR_OK) {
            $fileTmpPath = $_FILES['saladInput']['tmp_name'];
            $fileName = $_FILES['saladInput']['name'];
            $fileSize = $_FILES['saladInput']['size'];
            $fileType = $_FILES['saladInput']['type'];
            $fileNameCmps = explode(".", $fileName);
            $fileExtension = strtolower(end($fileNameCmps));
            $allowedfileExtensions = array('jpg');
            if (in_array($fileExtension, $allowedfileExtensions)) {
                $uploadFileDir = '../recipes/salads/images/';
