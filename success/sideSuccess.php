...
<body>
    <?php
        ...
        $recipe = $_FILES['sideInput']['name'];
        $splitRecipe = trim($recipe, ".jpg");
        $recipeInput = $db->prepare("INSERT INTO SideTable (id, Side) VALUES (?, ?)");
        $recipeInput->execute(array(null, $splitRecipe));
        echo '<h1>The Recipe: ' . $splitRecipe . ' Has Successfully Been Imported!</h1>';
        if (isset($_FILES['sideInput']) && $_FILES['sideInput']['error'] === UPLOAD_ERR_OK) {
            $fileTmpPath = $_FILES['sideInput']['tmp_name'];
            $fileName = $_FILES['sideInput']['name'];
            $fileSize = $_FILES['sideInput']['size'];
            $fileType = $_FILES['sideInput']['type'];
            $fileNameCmps = explode(".", $fileName);
            $fileExtension = strtolower(end($fileNameCmps));
            $allowedfileExtensions = array('jpg');
            if (in_array($fileExtension, $allowedfileExtensions)) {
                $uploadFileDir = '../recipes/sides/images/';
