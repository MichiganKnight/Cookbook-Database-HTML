...
<body>
    <?php
        ...
        $recipe = $_FILES['miscInput']['name'];
        $splitRecipe = trim($recipe, ".jpg");
        $recipeInput = $db->prepare("INSERT INTO MiscTable (id, Misc) VALUES (?, ?)");
        $recipeInput->execute(array(null, $splitRecipe));
        echo '<h1>The Recipe: ' . $splitRecipe . ' Has Successfully Been Imported!</h1>';
        if (isset($_FILES['miscInput']) && $_FILES['miscInput']['error'] === UPLOAD_ERR_OK) {
            $fileTmpPath = $_FILES['miscInput']['tmp_name'];
            $fileName = $_FILES['miscInput']['name'];
            $fileSize = $_FILES['miscInput']['size'];
            $fileType = $_FILES['miscInput']['type'];
            $fileNameCmps = explode(".", $fileName);
            $fileExtension = strtolower(end($fileNameCmps));
            $allowedfileExtensions = array('jpg');
            if (in_array($fileExtension, $allowedfileExtensions)) {
                $uploadFileDir = '../recipes/misc/images/';
