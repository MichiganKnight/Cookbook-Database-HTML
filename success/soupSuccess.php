...
<body>
    <?php
        ...
        $recipe = $_FILES['soupInput']['name'];
        $splitRecipe = trim($recipe, ".jpg");
        $recipeInput = $db->prepare("INSERT INTO soupTable (id, Soup) VALUES (?, ?)");
        $recipeInput->execute(array(null, $splitRecipe));
        echo '<h1>The Recipe: ' . $splitRecipe . ' Has Successfully Been Imported!</h1>';
        if (isset($_FILES['soupInput']) && $_FILES['soupInput']['error'] === UPLOAD_ERR_OK) {
            $fileTmpPath = $_FILES['soupInput']['tmp_name'];
            $fileName = $_FILES['soupInput']['name'];
            $fileSize = $_FILES['soupInput']['size'];
            $fileType = $_FILES['soupInput']['type'];
            $fileNameCmps = explode(".", $fileName);
            $fileExtension = strtolower(end($fileNameCmps));
            $allowedfileExtensions = array('jpg');
            if (in_array($fileExtension, $allowedfileExtensions)) {
                $uploadFileDir = '../recipes/soups/images/';
