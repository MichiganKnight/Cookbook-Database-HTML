...
<body>
    <?php
        ...
        $recipe = $_FILES['dessertInput']['name'];
        $splitRecipe = trim($recipe, ".jpg");
        $recipeInput = $db->prepare("INSERT INTO DessertTable (id, Dessert) VALUES (?, ?)");
        $recipeInput->execute(array(null, $splitRecipe));
        echo '<h1>The Recipe: ' . $splitRecipe . ' Has Successfully Been Imported!</h1>';
        if (isset($_FILES['dessertInput']) && $_FILES['dessertInput']['error'] === UPLOAD_ERR_OK) {
            $fileTmpPath = $_FILES['dessertInput']['tmp_name'];
            $fileName = $_FILES['dessertInput']['name'];
            $fileSize = $_FILES['dessertInput']['size'];
            $fileType = $_FILES['dessertInput']['type'];
            $fileNameCmps = explode(".", $fileName);
            $fileExtension = strtolower(end($fileNameCmps));
            $allowedfileExtensions = array('jpg');
            if (in_array($fileExtension, $allowedfileExtensions)) {
                $uploadFileDir = '../recipes/desserts/images/';
